...
`SuspensionAction`.** The gate checks the marker and calls `onSuspended` while `SUSPENDING` or
`SUSPENDED`, and `onResuming` while `RESUMING`. Events that accompany buffering belong in
...
- Every processor implementing `SuspensionAware` must explicitly classify commands for `SUSPENDING`
  and `SUSPENDED` (via `onSuspended`) and for `RESUMING` (via `onResuming`). The classifications still
  need to be reviewed together for consistency.
