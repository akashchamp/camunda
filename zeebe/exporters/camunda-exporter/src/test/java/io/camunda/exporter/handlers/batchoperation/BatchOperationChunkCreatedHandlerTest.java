...
class BatchOperationChunkCreatedHandlerTest {
  ...
  void shouldUpdateWithScriptOnFlush() throws PersistenceException {
    ...
    expectedParams.put("chunkRecordKeys", List.of(record.getKey()));
    expectedParams.put("chunkRecordItemCounts", List.of(1));
    expectedParams.put("maxChunkRecordKey", record.getKey());
    ...
    assertThat(script)
        .contains("((Number) candidate.partitionId).intValue() == params.partitionId");
    assertThat(script).contains("((Number) params.chunkRecordKeys[i]).longValue() > lastKey");
    assertThat(script)
  ...
  void shouldTrackDistinctRecordKeysAcrossMultipleUpdatesBeforeFlush() throws PersistenceException {
    ...
    final var recordKeys = (List<Long>) params.get("chunkRecordKeys");
    ...
    // the high-water-mark sent to the script covers every record of this flush cycle
    assertThat(params.get("maxChunkRecordKey"))
        .isEqualTo(Math.max(record1.getKey(), record2.getKey()));
  }
